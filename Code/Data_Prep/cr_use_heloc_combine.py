...
##################################################
#
# cr_use_heloc_combine constructs a dataset of HELOC balances.
...
##################################################
# Load trade-level HELOC datasets
##################################################
...
##################################################
# Combine HELOC datatsets into one file
##################################################
...
df_ind = spark.sql("SELECT TU_Consumer_ID, Run_date, \
                     SUM(CURRENT_BALANCE) AS heloc_bal, \
                     SUM(IF(MOP RLIKE '[4-5]', CURRENT_BALANCE, 0)) AS heloc_bal_arr, \
                     SUM(IF(MOP RLIKE '[7-9]', CURRENT_BALANCE, 0)) AS heloc_bal_def, \
                     SUM(CURRENT_BALANCE/n_now) AS heloc_bal_ind, \
                     SUM(IF(MOP RLIKE '[4-5]', CURRENT_BALANCE/n_now, 0)) AS heloc_bal_arr_ind, \
                     SUM(IF(MOP RLIKE '[7-9]', CURRENT_BALANCE/n_now, 0)) AS heloc_bal_def_ind, \
                     SUM(chargoff_refine) AS heloc_chargoff, \
                     SUM(chargoff_refine/n_now) AS heloc_chargoff_ind, \
...
df_ind = spark.sql("SELECT df_now.*, \
                     df_now.bal_excl_legacy - df_last.heloc_bal AS heloc_bal_chg, \
                     df_now.bal_excl_legacy_ind - df_last.heloc_bal_ind AS heloc_bal_chg_ind, \
                     df_now.chargoff_excl_legacy - df_last.heloc_chargoff AS heloc_chargoff_chg, \
                     df_now.chargoff_excl_legacy_ind - df_last.heloc_chargoff_ind AS heloc_chargoff_chg_ind \
...
df_ind_active = spark.sql("SELECT TU_Consumer_ID, Run_date, \
                            ...
                            SUM(cr_lmt/n_now) AS heloc_lmt_ind, \
                            SUM(cr_lmt*as115_origin_grp)/SUM(cr_lmt) AS heloc_crsc_grp, \
                            SUM(cr_lmt*as115_origin)/SUM(cr_lmt) AS heloc_crsc \
                          FROM df_acct \
...
df_ind = spark.sql("SELECT df_ind.*, \
                     df_ind_active.N_heloc, \
                     df_ind_active.heloc_lmt, df_ind_active.heloc_lmt_ind, \
                     df_ind_active.heloc_crsc_grp, df_ind_active.heloc_crsc \
                   FROM df_ind \

...
##################################################
#
# BoC_vs_TU_comp calculates the total aggregate credit-card balances
#   across all accounts and for chartered banks, for comparison with
#   regulatory figures from regulatory returns held at the Bank of Canada.
...
# Generate separate balance variable for big six banks. 
df_agg = spark.sql("SELECT df_acct.Run_date, \
                     df_acct.CURRENT_BALANCE, \
                     CASE WHEN L1 RLIKE 'Toronto Dominion Bank' \
                             OR L1 RLIKE 'Royal Bank of Canada' \
                             OR L1 RLIKE 'Canadian Imperial Bank of Canada' \
                             OR L1 RLIKE 'Bank of Nova Scotia' \
                             OR L1 RLIKE 'Bank of Montreal' \
                             OR L1 RLIKE 'National Bank' \
                        THEN CURRENT_BALANCE ELSE 0 AS CURRENT_BALANCE_BANK  \
                   FROM df_acct \
...
# MCP value to be filled in from regulatory returns. 
df_agg = spark.sql("SELECT Run_date, \
                     NULL AS MCP, \
                     SUM(CURRENT_BALANCE) AS tot_bal_all, \
                     SUM(CURRENT_BALANCE_BANK) AS tot_bal_bank \
                   FROM df_agg \
                   GROUP BY Run_date ")

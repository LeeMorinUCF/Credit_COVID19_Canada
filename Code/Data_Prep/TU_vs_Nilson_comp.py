...
df_agg = spark.sql("SELECT YEAR(df_acct.Run_date) AS year, \
                     df_acct.tu_consumer_id, \
                     df_acct.CURRENT_BALANCE, \
                     FN(df_acct.varname) AS ACTIVE_BALANCE \
                   FROM df_acct \
...
df_agg = spark.sql("SELECT Run_date, \
                     COUNT(tu_consumer_id) AS num_accts, \
                     SUM(CURRENT_BALANCE) AS tot_bal, \
                     SUM(ACTIVE_BALANCE) AS tot_active_bal \
                   FROM df_agg \
                   GROUP BY year ") \
         .where("Run_date >= DATE('2018-01-01')").where("Run_date < DATE('2020-01-01')")
